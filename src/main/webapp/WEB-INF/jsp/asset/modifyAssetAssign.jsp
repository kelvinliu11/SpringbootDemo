...
<div id="wrapper" style="width: 30%;margin:0 auto;margin-left: 200px;">
        ...
        <div class="table-responsive" style="width: 500px;">
          ...
          <table class="table">
            ...
            <tr>
              <td>分配日期:</td>
              <td><input type="text" style="width: 120px;" value="" id="datetimepickerAssignDate" data-date-format="yyyy-mm-dd hh:ii"></td>
            </tr>
            <tr>
              <td>分配资产额度:</td>
              <td><input class="form-control" value="${assetAssign.assignAmount}" style="display: inline;width: 200px;" id="assignAmount"></td>
            </tr>
            <tr>
              <td>年利率:</td>
              <td><input class="form-control" value="${assetAssign.yearRate}" style="display: inline;width: 200px;" id="yearRate"></td>
            </tr>
            <tr>
              <td>利息计算公式:</td>
              <td>资产包总额*年利率*（计息天数/<input class="form-control" value="${assetAssign.daysOfYear}" style="display: inline;width: 50px;" id="daysOfYear">）</td>
            </tr>
            <tr>
              <td>起息日期:</td>
              <td><input type="text" style="width: 120px;" value="" id="datetimepickerProfitCaculateDate" data-date-format="yyyy-mm-dd hh:ii"></td>
            </tr>
            <tr>
              <td>结息日期:</td>
              <td><input type="text" style="width: 120px;" value="" id="datetimepickerProfitContributeDate" data-date-format="yyyy-mm-dd hh:ii"></td>
            </tr>
            <tr>
              <td>备注</td>
              <td>
                <textarea class="form-control"value="${assetAssign.remark}" rows="3" id="remark">${assetAssign.remark}</textarea>
              </td>
            </tr>
            <tr>
              <td></td>
              <td>
                <button type="button" class="btn btn-default" onclick="modifyAssetAssign();">提交</button>
              </td>
            </tr>
            </tbody>
          </table>
...
<script language="JavaScript">
    $(document).ready(function() {
        ...
        var assignDate = "${assetAssign.assignDate}";
        var date1 = new Date(assignDate.replace(/-/g,"/"));
        $("#datetimepickerAssignDate").datetimepicker('setDate', date1);
        var profitCaculateDate = "${assetAssign.profitCaculateDate}";
        var date2 = new Date(profitCaculateDate.replace(/-/g,"/"));
        $("#datetimepickerProfitCaculateDate").datetimepicker('setDate', date2);
        var profitContributeDate = "${assetAssign.profitContributeDate}";
        var date3 = new Date(profitContributeDate.replace(/-/g,"/"));
        $("#datetimepickerProfitContributeDate").datetimepicker('setDate', date3);
    });
    function modifyAssetAssign() {
        $.ajax({
            type : "POST",  //提交方式
            url : "<%=request.getContextPath()%>/asset/moidfyAssetAssign/do",//路径
            async:false,
            data : {
                "id" : ${assetAssign.id},
