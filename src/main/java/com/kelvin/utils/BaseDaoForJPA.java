...
public abstract class BaseDaoForJPA {
    @PersistenceContext
		...
		/**
		 * 保存实体,实体必须使用符合JPA规范的注解进行配置
		 * 保存之后会给主键自动赋值(具体请查阅JPA EntityManager.persist 文档说明)
		 */
		 ...
		 * 保存之后会给主键自动赋值(具体请查阅JPA EntityManager.merge 文档说明)
		 */
		...
		/**
		 * 删除实体,实体必须使用符合JPA规范的注解进行配置 
		 * @param HQL 语言  <code>delete  User o  where o.id = ?</code>
		 */
